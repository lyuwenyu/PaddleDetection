...
def deformable_attention_core_func(value, value_spatial_shapes,
    ...
    if value_spatial_list is not None:
        value_list = value.split([h * w for h, w in value_spatial_list], axis=1)
    else:
    ...
    sampling_value_list = []
    for level, (h, w) in enumerate(value_spatial_list):
        # N_, H_*W_, M_, D_ -> N_, H_*W_, M_*D_ -> N_, M_*D_, H_*W_ -> N_*M_, D_, H_, W_

        # value_l_ = value_list[level].flatten(2).transpose(
        #     [0, 2, 1]).reshape([bs * n_head, c, h, w])
        value_l_ = value_list[level].flatten(2).transpose(
            [0, 2, 1]).reshape([-1, c, h, w])
    ...
    output = (paddle.stack(
        sampling_value_list, axis=-2).flatten(-2) *
              attention_weights).sum(-1).reshape([-1, n_head * c, Len_q])
