...
class DINOTransformerDecoder(nn.Layer):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            query_pos_embed = get_sine_pos_embed(reference_points_input[...,
                                                                        0, :])
            ...
            reference_points = F.sigmoid(bbox_head[i](output) + inverse_sigmoid(
                reference_points))
            ...
            if self.return_intermediate:
                ...
                intermediate_logits.append(dec_logit)

        if self.return_intermediate:
            return paddle.stack(intermediate), paddle.stack(
                intermediate_ref_points), paddle.stack(intermediate_logits)
...
class DINOTransformer(nn.Layer):
                ...
                nn.Sequential(
                    ('conv', nn.Conv2D(
                        in_channels, self.hidden_dim, kernel_size=1)), (
                            'norm', nn.GroupNorm(32, self.hidden_dim))))
            ...
            self.input_proj.append(
                nn.Sequential(
                    ('conv', nn.Conv2D(
                        ...
                        kernel_size=3,
                        stride=2,
                        padding=1)), ('norm', nn.GroupNorm(32, self.hidden_dim)
                                      )))
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None):
        ...
        target, init_ref_points, enc_topk_bboxes, enc_topk_logits = self._get_decoder_input(
