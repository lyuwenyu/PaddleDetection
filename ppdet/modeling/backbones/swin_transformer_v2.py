...
class WindowAttention(nn.Layer):
    ...
    def __init__(self,
        ...
        self.logit_scale = add_parameter(
            self, paddle.log(10 * paddle.ones([num_heads, 1, 1])))
        # self.logit_scale.stop_gradient = False
        ...
        relative_coords_table = paddle.stack(
            paddle.meshgrid([relative_coords_h, relative_coords_w])).transpose(
                [1, 2, 0]).unsqueeze(axis=0)  # 1, 2*Wh-1, 2*Ww-1, 2
        ...
        self.relative_coords_table = paddle.sign(
        ...
        if qkv_bias:
            self.q_bias = add_parameter(self, paddle.zeros([dim]))
            self.v_bias = add_parameter(self, paddle.zeros([dim]))
        else:
            self.q_bias = None
            self.v_bias = None
    ...
    def forward(self, x, mask=None):
        ...
        qkv = qkv.reshape(
            [B_, N, 3, self.num_heads, -1]).transpose([2, 0, 3, 1, 4])
        q, k, v = qkv[0], qkv[1], qkv[2]

        attn = paddle.mm(F.normalize(
            q, axis=-1),
                         F.normalize(
                             k, axis=-1).transpose([0, 1, 3, 2]))
        ...
        logit_scale = paddle.clip(
            self.logit_scale, max=paddle.log(paddle.to_tensor(1. / 0.01))).exp()
...
class BasicLayer(nn.Layer):
    ...
    def forward(self, x, H, W):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                x = paddle.distributed.fleet.utils.recompute((blk,
                                                              (x, attn_mask)))
            else:
                x = blk(x, attn_mask)
    ...
    def _init_respostnorm(self):
        for blk in self.blocks:
            zeros_(blk.norm1.bias)
            zeros_(blk.norm1.weight)
            zeros_(blk.norm2.bias)
            zeros_(blk.norm2.weight)
